...
namespace :data_migration do
  ...
  task :rename_alert_subscription_lists, %i[from_slug to_slug] => :environment do |_t, args|
    SubscriberList.where("tags->'alert_type' IS NOT NULL").all.each do |list|
      ...
      if (new_list = SubscriberList.where("tags->'alert_type' IS NOT NULL").find_all { |l| l.tags[:alert_type][:any].sort == new_tags.sort }.first)
